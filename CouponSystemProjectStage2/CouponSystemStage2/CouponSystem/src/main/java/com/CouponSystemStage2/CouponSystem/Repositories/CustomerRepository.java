...
public interface CustomerRepository extends JpaRepository<Customer, Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value = "update customers set first_name= :#{#customer.firstName}, last_name= :#{#customer.lastName}, password = :#{#customer.password}, email = :#{#customer.email} where id= :#{#customer.id}", nativeQuery = true)
    void updateCustomer(@Param("customer") Customer customer);

    Customer getByEmailAndPassword(String email, String password);

    @Query(value = " SELECT CASE WHEN COUNT(*) > 0 THEN 'true' ELSE 'false' END from coupons_vs_customers cc where cc.coupon_id= :couponId AND cc.customer_id = :customerId", nativeQuery = true)

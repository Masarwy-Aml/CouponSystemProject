...
public interface CouponRepository extends JpaRepository<Coupon, Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value = "update coupons set title= :#{#coupon.title}, start_date= :#{#coupon.startDate}, end_date = :#{#coupon.endDate}, category_id= :#{#coupon.category.ordinal}, image= :#{#coupon.image}, price= :#{#coupon.price}, description = :#{#coupon.description}, amount = :#{#coupon.amount} where id= :#{#coupon.id}", nativeQuery = true)
    void updateCoupon(@Param("coupon") Coupon coupon);

    @Query(value = " select * from coupons_vs_customers t1 join coupons t2 ON t1.coupon_id=t2.id where customer_id= :customerId ", nativeQuery = true)
